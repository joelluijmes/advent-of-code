...
def accumulate_removal(grid: list[str]):
    """Keep removing rolls until stabilizes. Returns total removed paper rolls."""
    previous_value, removed_rolls = 0, None

    while previous_value != removed_rolls:
        previous_value = removed_rolls

        grid = find_accessible_spots(grid)
        removed_rolls = sum(sum(1 for col in row if col == ACCESSIBLE) for row in grid)
...
def main():
    ...
    result = accumulate_removal(grid)
